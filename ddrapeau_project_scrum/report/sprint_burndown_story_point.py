...
class report_tasks(report_int):
    def create(self, cr, uid, ids, datas, context=None):
        ...
        tb = text_box.T(loc=(320, 500), text="/hL/15/bBurndown Chart (points)", line_style=None)
        tb.draw()
        ...
        for sprint in sprint_pool.browse(cr, uid, ids, context=context):
            total_story_ids = userStory_pool.search(cr, uid, [('sprint_id','=',sprint.id)], context=context)
            for story_id in total_story_ids:
                total_points += userStory_pool.read(cr, uid, story_id, ['complexity'])['complexity']
            
            done_story_ids = userStory_pool.search(cr, uid, [('sprint_id','=',sprint.id), ('state', '=', 'done')], context=context)
            datas = _burndown_story_point.compute_burndown(cr, uid, done_story_ids, sprint.date_start, sprint.date_stop, total_points)
            max_points = reduce(lambda x,y: max(y[1],x), datas, 0)
            ...
            guideline__data=[(datas[0][0],total_points), (datas[-1][0],0)]
